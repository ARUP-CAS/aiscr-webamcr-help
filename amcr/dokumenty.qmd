...
[Dokumenty](/slovnik/index.qmd#dokumenty) do AMČR nejčastěji připojujeme jako součásti projektových akcí, tedy nálezové zprávy apod., většina operací s dokumenty tedy probíhá v rámci správy projektů, resp. akcí nebo lokalit či na ně přímo navazuje. 
...
Zde je možné prohlížet a editovat již existující záznamy. 
Přidávat zcela nové záznamy dokumentů je možné pouze skrze moduly `Projekty`, `Akce` a `Lokality`.
...
![](figs/dok05.png){width="42%" fig-align="center"}

 
### Zápis nového dokumentu

Při kliknutí na tlačítko `+` v tabulce `Dokumenty` v modulech `Akce`, `Projekty` nebo `Lokality` je třeba vybrat z rolovací nabídky možnost `Přidat dokument`. 
Aplikace se tím přepne do editačního prostředí modulu `Dokumenty`.

![](figs/dok06.png){width="50%" fig-align="center"}

![](figs/dok07.png){width="50%" fig-align="center"}

![](figs/dok08.png){width="50%" fig-align="center"}
...
Po vytvoření dokumentu je možné vyplnit jeho metadatový popis. 
...
```{r}
#| tbl-cap: "Licence, které lze u dokumentů zvolit"
cc <- read.csv("tabs/licenses.csv", tryLogical = FALSE)
...
```{r}
#| tbl-cap: "Přijímané formáty souborů"
cnames <- c("", "Formát", "Archivní formát", "Vhodné pro", "Doporučení", "Poznámka")
# icons <- c("img" = "{{< bi card-image >}}", "tbl" = "{{< bi table >}}", "doc" = "{{< bi card-text >}}")
icons <- c("img" = "file-earmark-image", "tbl" = "file-earmark-ruled", "doc" = "file-earmark-text")

formats <- read.csv("tabs/fileformats.csv", tryLogical = FALSE)
# archival format - tick, non-archival format - cross
formats[formats$archival == "TRUE", ]$archival <- "**&#x2714;**" 
# formats[formats$archival == "FALSE", ]$archival <- "**&#x2718;**"
# icons 
formats$icon <- paste0("{{< bi ", unname(icons[formats$icon]), " size=1.2em >}}")

formats |> 
  knitr::kable(align = "ccclll", col.names = cnames)

# formats |>
# dplyr::mutate(archival = dplyr::if_else(archival == TRUE, "&#x2714;", "&#x2718;"),
#               format = paste0("**", format, "**"),
#               icon =  paste0("{{< bi ", icons[icon], " size=1.2em >}}")) |>
# knitr::kable(align = "ccclll", col.names = cnames)
```

:::

Optimalizace PDF do archviního formátu PDF/A-2b je prováděna pomocí Adobe Acrobat s využitím funkce "Kontrola před výstupem". 
Přesná specifikace a export nastavení jsou dostupné v [tomto archivu {{< bi file-earmark-arrow-down >}}](docs/AMCR_PDF_A2b_nastaveni.zip).
