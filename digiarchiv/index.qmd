...
- **{{< bi house-door-fill >}} Home** -- návrat na hlavní stránku;
- **{{< bi question-circle >}} Nápověda** -- otevře tuto nápovědu;
- **{{< bi box-arrow-right >}} Přihlásit uživatele** -- registrovaní uživatelé se zde mohou přihlásit a získají tak přístup ke všem datům, podle své. uživatelské role;
- **{{< bi person-check-fill >}} Registrovat uživatele** -- neregistrovaní uživatelé zde mohou provést registraci;
- **{{< bi graph-up >}} Statistiky** -- statistiky přístupů k jednotlivým záznamům.
- **English** -- přepnout do anglické verze webu.


### Uživatelské účty

**Registrovaní uživatelé** se mohou přihlásit svým uživatelským jménem a heslem; údaje jsou shodné jako při užití aplikace [AMČR](https://amcr.aiscr.cz/){.external}. Neregistrovaní uživatelé se mohou registrovat kliknutím na možnost [Registrovat uživatele](https://amcr.aiscr.cz/accounts/register/){.external}, která otevře stránku s registračním formulářem.
...
Informace o zpracování osobních údajů registrovaných uživatelů naleznete [zde](/zakladni-info/osobni-udaje.qmd).
...
Pro fulltextové vyhledávání lze použít pole **{{< bi search >}} Zadejte vyhledávácí dotaz**.
...
Tlačítko **{{< bi map-fill >}} Otevřít mapu** slouží pro přesun do mapového okna. 
...
Výsledky lze zobrazit buď v **{{< bi display >}} Plném zobrazení** (titulek, náhled, popis), jako **{{< bi grid-3x3-gap-fill >}} Galerii** (pouze náhledy), či v **{{< bi justify >}} Řádkovém zobrazení** (pouze titulek). 
Vybrané záznamy lze exportovat jako seznam pomocí tlačítka **{{< bi download >}} Export**. 
...
- **{{< bi map-fill >}} Zobrazit na mapě** -- otevře mapové okno, pokud je to pro záznam relevantní.
- **{{< bi link >}} Odkazy** -- umožňuje získat permalink či odkaz na záznam v [OAI-PMH API](https://api.aiscr.cz/){.external} (více viz dále).
- **{{< bi book-half >}} BibTeX** -- vygeneruje bibliografický záznam v BibTeX, který vytvoří soubor s příponou .bib a který lze použít k tvorbě bibliografie v externích nástrojích (např. Zotero).
- **{{< bi printer >}} Vytisknout** -- umožňuje tisk detailu daného záznamu.
- **{{< bi star-fill >}} Uložit do oblíbených** -- označí záznam jako oblíbený (pouze pro přihlášené uživatele). Oblíbené záznamy je možné zobrazit pomocí totožného symbolu v záhlaví stránky s výsledky.
- **{{< bi chat-left-dots-fill >}} Odeslat zpětnou vazbu** -- umožňuje odeslat komentář.
- **Zobrazit detail {{< bi chevron-down >}}** -- otevře detail záznamu.


### Detail záznamu

U každého ze zobrazených výsledků lze **Zobrazit detail {{< bi chevron-down >}}** záznamu, který obsahuje podrobný popis výsledku a odkazy na související záznamy. 
...
U detailu projektu jsou u polí vázaných na řízené slovníky (hesláře) symboly filtrace **{{< bi funnel-fill >}}**. 
Po kliknutí na takto označené pole se konkrétní zvolené heslo použije jako aktivní filtr (k použití filtrů viz níže).

Každý identifikátor v detailu záznamu je opatřen ikonou odkazu **{{< bi link >}}**, která umožňuje zobrazit samostatnou stránku persistentního odkazu se souvisejícími daty. 
...
Soubory lze pomocí příslušného tlačítka stahovat **{{< bi download >}}**. 
Generování náhledů souborů probíhá zpětně po jejich publikaci v Digitálním archivu. 
Pokud u souboru ještě není vygenerován jeho náhled, zobrazí se místo něj ikona souboru bez náhledu **{{< bi file-earmark-fill >}}**. 
...
Na tuto stránku se lze dostat vždy po kliknutí na symbol odkazu **{{< bi link >}}** kdekoliv v rámci aplikace. 
Persistentní odkaz se nachází v ovládacích prvcích pod každým výsledkem či jeho popisem a u souvisejících záznamů po rozbalení nabídky **Zobrazit detail {{< bi chevron-down >}}**. 
...
Po zvolení pole, podle kterého chceme vyhledávat, je třeba vyplnit hledanou hodnotu či rozsah hodnot (např. hledané rozpětí data výzkumu). Pro aplikaci zadaného filtru slouží tlačítko **{{< bi funnel-fill >}} Aplikovat filtr**.
...
Ve filtrech je možné vyhledávat přímo konkrétní heslo v konkrétním hesláři, nebo je možné použít pole **Vyhledej pojem v heslářových filtrech {{< bi search >}}**.
...
U hodnot použitelných pro filtraci je ikona pro aplikaci filtru **{{< bi funnel-fill >}}**. 
...
Do mapového okna se lze dostat buď pomocí tlačítka **{{< bi map-fill >}} Otevřít mapu** na horním panelu nebo pomocí tlačítka **{{< bi map-fill >}} Zobrazit na mapě** u konkrétního záznamu.

V mapovém okně máme po levé straně pomocí tlačítka **{{< bi chevron-bar-right >}} Filtrace/Výsledky** možnost vysunout panel, kde lze zobrazit výsledky a pracovat s filtry. 
Použití filtrů je totožné jako v zobrazení stránky s výsledky. 
Panel je možné pro větší přehlednost skrýt **{{< bi chevron-bar-left >}}**. 
Rovněž zde platí možnost použít fulltextové vyhledávání.

Většinu zobrazení zabírá mapové okno se zobrazenými výsledky. 
Pro pohyb v mapě slouží myš a pro změnu měřítka tlačítka **{{< bi plus >}}** / **{{< bi dash >}}** či kolečko myši. 
...
Symboly **{{< bi geo-alt-fill >}}** reprezentují jejich definiční body. 
...
Po kliknutí na ikonu **{{< bi stack >}}** v pravém horním rohu mapového okna je možné změnit mapový podklad. 
Na výběr je ČÚZK – Základní mapa ČR, ČÚZK – Ortofotomapa a ČÚZK – Stínovaný reliéf 5G a dále [OpenStreetMap](https://www.openstreetmap.org/){.external}.
K těmto podkladovým mapám je dále možné jako poloprůhledný překryv doplnit mapu ČÚZK – Katastrální území a ČÚZK – Katastrální mapu (dostupné podle zvoleného měřítka - mapu je nutno dostatečně přiblížit).

![](figs/da-map8.png){.column-margin width=200}

Vybrané záznamy je možné pomocí tlačítka **{{< bi download >}} Exportovat seznam** exportovat a to ve formátech `GML`, `WKT` a `GeoJSON`, se kterými je možné dále pracovat např. v prostředí GIS (prostorové údaje v těchto formátech jsou uloženy v jednom z polí exportní tabulky; samostatně jsou vždy uváděny též souřadnice centroidu ve WGS-84). 
